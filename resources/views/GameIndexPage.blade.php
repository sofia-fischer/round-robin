...
<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto flex-grow">
        ...
        @if( \Illuminate\Support\Facades\Auth::user()->email)
            @foreach($gameLogics as $gameLogic)
                <form action="/game"
                    method="POST"
                    class="text-white text-center py-2 px-4 m-2 font-semibold rounded-full bg-gradient-to-r from-indigo-400 to-pink-500 hover:from-blue-500 hover:to-green-500">
                    @csrf
                    <button name="logic" value="{{ $gameLogic }}"> Start a new Game {{ $gameLogic::title() }}</button>
                </form>
            @endforeach
        @endif
    </div>
